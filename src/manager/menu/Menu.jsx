...
export default class Menu extends ShallowComponent {
        ...
        return (
            <PageCard header="Menü Yönetimi">
                <DataGrid
                    ...
                    onEditClick={this.__removeData}
                    onDeleteClick={this.__removeData}
    ...
    __removeData(e:Object){
        let object = [];
        object = this.refs.table.state.rows;
        let objectData =[];
        for(let item in object){
            let name = object[item].text;
            objectData.push({name : name});
        }

        let model = this.state.fields;
        for(let item in model){
            let name = model[item].label;
            object.push({name : name});
        }
